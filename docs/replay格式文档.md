...
**对象格式（当前）**：每天第一个回合发布一次，后续回合为空字符串：

| 字段 | 类型 | 说明 |
| ---- | ---- | ---- |
| `officialNews` | `string` | 官方消息（推理类任务，可能影响小贩回收价）。`"今日无重大新闻"` 表示无事件 |
| `folkLegends` | `string` | 民间传闻（长上下文任务线索，多天线索需累积）。空串表示当天无传闻 |

```json
"news": {
  "officialNews": "矿业管理局紧急通报：北部铁矿区昨夜发生严重矿井塌方事故…",
  "folkLegends": "村里最年长的采药人昨天过世了，活了九十三岁…"
}
```

**旧数组格式（兼容）**：`news` 为 `object[]`，元素为 `{"type","text"}` 或纯字符串，记录击杀/任务完成等关键事件。
